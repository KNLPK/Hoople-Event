...
export function OrgSidebar({ onClose }: { onClose?: () => void }) {
  ...
  const [paymentsOpen, setPaymentsOpen] = useState(pathname.startsWith('/organizer/payments'));

  return (
    <aside className="org-side">
      <Link to="/organizer" className="org-side__brand" aria-label="Hoople organizer console">
      ...
      <Link to="/organizer/settings" className="org-workspace">
        ...
        <span className="org-workspace__text">
          ...
          <span className="org-workspace__category">{WORKSPACE.category}</span>
        </span>
      ...
      <nav className="org-nav">
        ...
        {paymentsOpen ? (
          <div className="org-nav__children">
            ...
            <NavLink to="/organizer/payments/transactions" className={navClass}>
              Transactions
            </NavLink>
          </div>
        ) : null}
      ...
      <div className="org-upgrade">
        ...
        <div className="org-upgrade__art">
          <ImageSlot id="org-upgrade-art" shape="rounded" radius={12} placeholder="Mascot" />
        </div>
      ...
      <Link to="/help" className="org-side__help">
        ...
        Help Center
        <ExternalLink size={15} color="#8B8A99" strokeWidth={1.9} style={{ marginLeft: 'auto' }} />
      </Link>
    </aside>
  );
}
